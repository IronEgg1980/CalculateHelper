    ...
    public List<AssignGroupByPerson> getAssignGroupByPersonList(LocalDateTime localDateTime){
        ...
        if(pepole!=null){
            for(String person : pepole){
                List<AssignDetails> findList = find(localDateTime,person);
                if(findList.isEmpty())
                    continue;
                ...
                StringBuilder monthNote = new StringBuilder();
                ...
                assignGroupByPerson.setOffDaysNote(monthNote.substring(1));
